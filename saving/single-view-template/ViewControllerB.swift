...
class ViewControllerB: UIViewController {
    ...
    var item: TableSectionItem? = .none
    ...
    func setupNavigationBar() {
        title = section == nil ? "Create todo" : item?.created.components(separatedBy: " ")[1]
    ...
    func setupSegmentedControl() {
        ...
        segment.selectedSegmentIndex = s
    ...
    @objc func save() {
        ...
        if var i = item {
            i.string = s
            i.image = imagePngData
            delegate?.didPressSave(section: t, item: i)
            navigationController?.popViewController(animated: true)
            return
        }
        
        let i = TableSectionItem(image: imagePngData, string: s, created: String(describing: Date()))
        delegate?.didPressSave(section: t, item: i)
        navigationController?.popViewController(animated: true)
    }
